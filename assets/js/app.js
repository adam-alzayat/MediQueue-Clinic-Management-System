(function () {
  ...
  const data = () => apiData || window.MQData;
  ...
  async function fetchApiResource(label, path) {
    try {
      const response = await fetch(`${API_BASE}${path}`);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const payload = await response.json();
      console.log(`[MediQueue] ${label} loaded from API`);
      return payload;
    } catch (error) {
      console.log(`[MediQueue] ${label} using local fallback (${error.message})`);
      return null;
    }
  }

  async function postAppointmentToApi(booking) {
    if (!window.fetch) {
      return { fallback: true, error: "Fetch is unavailable" };
    }
    ...
    try {
      ...
      if (!response.ok) {
        return { ok: false, error: payload.error || "Booking could not be created" };
      }

      const appointment = payload.appointment || payload;
      console.log(`[MediQueue] booking created through API: ${appointment.reference}`);
      return { ok: true, appointment };
    } catch (error) {
      console.log(`[MediQueue] booking API unavailable, using local fallback (${error.message})`);
      return { fallback: true, error: error.message };
    }
  }

  async function refreshAppointmentsFromApi() {
    const appointments = await fetchApiResource("appointments", "/appointments");
    if (!appointments) return;
    apiData = { ...(apiData || window.MQData), appointments };
  }

  async function loadApiData() {
    if (!window.fetch) {
      console.log("[MediQueue] Fetch is unavailable, using local fallback data");
      return;
    }

    const [apiServices, apiDoctors, apiPatients, apiAppointments, apiQueue, apiFinance] = await Promise.all([
      fetchApiResource("services", "/services"),
      fetchApiResource("doctors", "/doctors"),
      fetchApiResource("patients", "/patients"),
      fetchApiResource("appointments", "/appointments"),
      fetchApiResource("queue", "/queue"),
      fetchApiResource("finance", "/finance"),
    ]);

    apiData = {
      ...window.MQData,
      services: apiServices || window.MQData.services,
      doctors: apiDoctors || window.MQData.doctors,
      patients: apiPatients || window.MQData.patients,
      appointments: apiAppointments || window.MQData.appointments,
      queue: apiQueue || window.MQData.queue,
      expenses: apiFinance?.expensesList || window.MQData.expenses,
      payroll: apiFinance?.payroll || window.MQData.payroll,
    };
  }
  ...
  function sortedQueue() {
    return [...queueEntries()].sort((a, b) => {
      ...
      return joinedAtValue(a.joinedAt) - joinedAtValue(b.joinedAt);
    });
  }

  function joinedAtValue(value) {
  ...
  function initBooking() {
    ...
    byId("booking-form").addEventListener("submit", async (event) => {
      ...
      const apiResult = await postAppointmentToApi(booking);
      let confirmedBooking = booking;

      if (apiResult.ok) {
        confirmedBooking = apiResult.appointment;
        ...
        showMessage("booking-result", apiResult.error, "danger");
        renderSlots();
        return;
      }

      addNotification("patient", `Booking confirmed: ${confirmedBooking.reference} for ${serviceById(confirmedBooking.serviceId).name}.`, "success");
      showMessage("booking-result", `Booking confirmed. Reference ${confirmedBooking.reference}. Please arrive 10 minutes early.`, "success");
      event.target.reset();
      dateInput.value = today();
      renderDoctorOptions(serviceSelect.value, doctorSelect);
      renderSlots();
